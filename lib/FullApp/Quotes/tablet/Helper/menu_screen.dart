...
class _MenuViewState extends State<MenuView> with TickerProviderStateMixin {
    ...
    for (var i = 0; i < widget.menu.items.length; ++i) {
      ...
      if (widget.animation)
        listItems.add(new AnimatedMenuListItem(
          ...
          curve: new Interval(animationIntervalStart / maxDuration, animationIntervalEnd / maxDuration, curve: Curves.easeOut),
  ...
  Widget build(BuildContext context) {
    ...
    textStyle = widget?.textStyle ?? Theme.of(context).textTheme.subtitle2.copyWith(color: widget.color.computeLuminance() < 0.5 ? Colors.white : Colors.black);
    return new DrawerScaffoldMenuController(builder: (BuildContext context, MenuController menuController) {
        ...
        child: new Material(
          ...
          child: new Stack(
            children: [
              ...
              widget.animation && shouldRenderSelector ? new ItemSelector(selectorColor: selectorColor, topY: actualSelectorYTop, bottomY: actualSelectorYBottom, opacity: selectorOpacity) : new Container(),
            ],
          ),
        ),
      );
...
class _AnimatedMenuListItemState extends AnimatedWidgetBaseState<AnimatedMenuListItem> {
  ...
  updateSelectedRenderBox(bool useState) {
    ...
    if (renderBox != null && widget.isSelected) {
      (menuScreenKey.currentState as _MenuViewState).setSelectedRenderBox(renderBox, useState);
    }
  }
...
class _MenuListItem extends StatelessWidget {
  ...
  _MenuListItem({this.title, this.isSelected, this.onTap, this.menuView, @required this.textStyle, @required this.selectorColor, this.icon, this.drawBorder});
