...
import 'package:smartkit/FullApp/NewsApp/desktop/helper/VideoView/overlay_service.dart';
import 'package:smartkit/FullApp/NewsApp/model/LiveVideo.dart';

LiveVideo selectedlivevideo;

class LiveVideoListDesktop extends StatefulWidget {
  @override
  _LiveVideoListStateDesktop createState() => _LiveVideoListStateDesktop();
}

class _LiveVideoListStateDesktop extends State<LiveVideoListDesktop> {
  ...
  Widget videoListWidget() {
    return GridView.count(
        ...
        children: List.generate(livevideolist.length, (index) {
          LiveVideo video = livevideolist[index];
          return ButtonClickAnimation(
            onTap: () {
              selectedlivevideo = video;

              OverlayService().addVideosOverlay(context, LiveVideoDetailDesktop());
            },
            child: Card(
