...
class SlidableDelegateContext {
  ...
  List<Widget> buildActions(BuildContext context) {
    return List.generate(state.actionCount, (int index) => state.actionDelegate.build(context, index, state.actionsMoveAnimation, SlidableRenderingMode.slide));
  }

  Offset createOffset(double value) {
    return state.directionIsXAxis ? new Offset(value, 0.0) : new Offset(0.0, value);
  }
...
class Slidable extends StatefulWidget {
  ...
  Slidable.builder({
    ...
    this.controller,
        ...
        assert(showAllActionsThreshold != null && showAllActionsThreshold >= .0 && showAllActionsThreshold <= 1.0, 'showAllActionsThreshold must be between 0.0 and 1.0'),
...
class SlidableState extends State<Slidable> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin<Slidable> implements SlidableStateView {
  ...
  void initState() {
    super.initState();
    _overallMoveController = new AnimationController(duration: widget.movementDuration, vsync: this)..addStatusListener(_handleDismissStatusChanged);
    _actionsMoveController = new AnimationController(duration: widget.movementDuration, vsync: this)..addStatusListener(_handleShowAllActionsStatusChanged);
  }
  ...
  @override
  bool get wantKeepAlive => !widget.closeOnScroll && (_overallMoveController?.isAnimating == true || _actionsMoveController?.isAnimating == true || _resizeController?.isAnimating == true);
  ...
  void _startResizeAnimation() {
    ...
    if (slideToDismissDelegate.resizeDuration == null) {
      ...
      _resizeController = new AnimationController(duration: slideToDismissDelegate.resizeDuration, vsync: this)
      ...
      setState(() {
        ...
        _resizeAnimation = new Tween<double>(begin: 1.0, end: 0.0).animate(new CurvedAnimation(parent: _resizeController, curve: _kResizeTimeCurve));
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.enabled || ((widget.actionDelegate == null || widget.actionDelegate.actionCount == 0) && (widget.secondaryActionDelegate == null || widget.secondaryActionDelegate.actionCount == 0))) {
      return widget.child;
    }
        ...
        if (_resizeAnimation != null) {
          ...
          assert(() {
            if (_resizeAnimation.status != AnimationStatus.forward) {
              ...
              throw new FlutterError('A dismissed Slidable widget is still part of the tree.\n'
