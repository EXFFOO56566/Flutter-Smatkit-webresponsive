...
class DragAndDropList extends StatefulWidget {
  ...
  DragAndDropList(this.rowsCount, {Key key, @required this.itemBuilder, this.onDragFinish, @required this.canBeDraggedTo, this.dragElevation = 0.0, this.canDrag, scrollController})
...
class _DragAndDropListState extends State<DragAndDropList> {
  ...
  void _maybeScroll() {
    ...
    if (shouldScrollUp) {
      ...
      widget.scrollController.animateTo(scrollTo, duration: new Duration(milliseconds: 74), curve: Curves.linear).then((it) {
        updatePlaceholder();
        isScrolling = false;
        _maybeScroll();
      });
    ...
    if (shouldScrollDown) {
      ...
      isScrolling = true;
      var scrollTo = widget.scrollController.offset + 12.0;
      widget.scrollController.animateTo(scrollTo, duration: new Duration(milliseconds: 75), curve: Curves.linear).then((it) {
        updatePlaceholder();
        isScrolling = false;
        _maybeScroll();
      });
...
//        double end = rend.localToGlobal(new Offset(0.0, rend.semanticBounds.height)).dy;
  ...
  void updatePlaceholder() {
    ...
    if (nextIndex < rows.length && previousIndex == nextIndex && rows[nextIndex].isExtraAtTop == atTop && rows[nextIndex].extraHeight == dragHeight) {
      return;
    }
    ...
    if (nextIndex < rows.length) {
      if (dragHeight != null && (absMinus(rows[nextIndex].extraHeight, dragHeight) > 0.1 || rows[nextIndex].isExtraAtTop != atTop)) {
