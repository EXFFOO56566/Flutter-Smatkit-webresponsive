...
class FlutterSlider extends StatefulWidget {
  ...
  final Function(int handlerIndex, double lowerValue, double upperValue) onDragStarted;
...
class _FlutterSliderState extends State<FlutterSlider> with TickerProviderStateMixin {
  ...
  double __dAxis, __rAxis, __axisDragTmp, __axisPosTmp, __containerSizeWithoutPadding, __rightHandlerPosition, __leftHandlerPosition, __containerSizeWithoutHandlerSize, __middle, __containerSizeWithoutHalfPadding, __handlerSize;
  ...
  void _setParameters() {
    ...
    _tooltipData.boxStyle ??= FlutterSliderTooltipBox(decoration: BoxDecoration(border: Border.all(color: Colors.black12, width: 0.5), color: Color(0xffffffff)));
  ...
  void _arrangeHandlersPosition() {
    if (widget.axis == Axis.horizontal) {
      ...
      _leftHandlerXPosition = (((_constraintMaxWidth - _handlersWidth) / _fakeMax) * _lowerValue) - (_touchSize);
      _rightHandlerXPosition = (((_constraintMaxWidth - _handlersWidth) / _fakeMax) * _upperValue) - (_touchSize);
    } else {
      _handlersPadding = _handlersHeight / 2;
      _leftHandlerYPosition = (((_constraintMaxHeight - _handlersHeight) / _fakeMax) * _lowerValue) - (_touchSize);
      _rightHandlerYPosition = ((_constraintMaxHeight - _handlersHeight) / _fakeMax) * _upperValue - (_touchSize);
    }
  }
  ...
  void initState() {
    ...
    _leftHandlerScaleAnimation = Tween(begin: 1.0, end: widget.handlerAnimation.scale).animate(CurvedAnimation(parent: _leftHandlerScaleAnimationController, reverseCurve: widget.handlerAnimation.reverseCurve, curve: widget.handlerAnimation.curve));
    _rightHandlerScaleAnimation = Tween(begin: 1.0, end: widget.handlerAnimation.scale).animate(CurvedAnimation(parent: _rightHandlerScaleAnimationController, reverseCurve: widget.handlerAnimation.reverseCurve, curve: widget.handlerAnimation.curve));
  ...
  _initialize(_) {
    ...
    if (widget.rangeSlider == true && _finalLeftHandlerWidth != _finalRightHandlerWidth) {
      throw 'ERROR: Width of both handlers should be equal';
    }

    if (widget.rangeSlider == true && _finalLeftHandlerHeight != _finalRightHandlerHeight) {
  ...
  void didUpdateWidget(FlutterSlider oldWidget) {
    ...
    bool upperValueEquality = (widget.values.length > 1 && oldWidget.values[1] != widget.values[1]);
  ...
  void _validations() {
    ...
    if (widget.values[0] != null && widget.values[0] < widget.min) throw 'Lower value should be greater than min';

    if (widget.rangeSlider == true) {
      if (widget.values[1] != null && widget.values[1] > widget.max) throw 'Upper value should be smaller than max';
    }
  }
  ...
  void _generateHandler() {
    ...
    if (widget.rightHandler != null && (widget.rightHandler.icon != null || widget.rightHandler.child != null)) tmpInputRightHandler = widget.rightHandler;

    rightHandler = _MakeHandler(
      animation: _rightHandlerScaleAnimation,
      id: rightHandlerKey,
      visibleTouchArea: widget.visibleTouchArea,
      handlerData: tmpInputRightHandler ?? FlutterSliderHandler(icon: Icon((widget.axis == Axis.horizontal) ? Icons.chevron_left : Icons.expand_less, color: Colors.black45)),
    ...
    /*Left Handler Data*/
    IconData hIcon = (widget.axis == Axis.horizontal) ? Icons.chevron_right : Icons.expand_more;
    if (widget.rtl && !widget.rangeSlider) {
      hIcon = (widget.axis == Axis.horizontal) ? Icons.chevron_left : Icons.expand_less;
    }

    FlutterSliderHandler tmpInputHandler;
    if (widget.handler != null && (widget.handler.icon != null || widget.handler.child != null)) tmpInputHandler = widget.handler;

    leftHandler = _MakeHandler(
        animation: _leftHandlerScaleAnimation,
        id: leftHandlerKey,
        visibleTouchArea: widget.visibleTouchArea,
        handlerData: tmpInputHandler ?? FlutterSliderHandler(icon: Icon(hIcon, color: Colors.black45)),
        width: _handlersWidth,
        height: _handlersHeight,
        axis: widget.axis);
  ...
  void _leftHandlerMove(PointerEvent pointer, [double tappedPositionWithPadding = 0]) {
    ...
    if (widget.axis == Axis.horizontal) {
      ...
      __containerSizeWithoutHandlerSize = _constraintMaxHeight - _handlersHeight;
    ...
    __rAxis = ((__axisPosTmp / (__containerSizeWithoutPadding / _divisions)) * widget.step);
    __rAxis = (double.parse(__rAxis.toStringAsFixed(_decimalScale)) - double.parse((__rAxis % widget.step).toStringAsFixed(_decimalScale)));

    if (widget.rangeSlider && widget.minimumDistance > 0 && (__rAxis + widget.minimumDistance) >= _upperValue) {
      _lowerValue = (_upperValue - widget.minimumDistance > _fakeMin) ? _upperValue - widget.minimumDistance : _fakeMin;
      validMove = false;
      _updateLowerValue(_lowerValue);
    }

    if (widget.rangeSlider && widget.maximumDistance > 0 && __rAxis <= (_upperValue - widget.maximumDistance)) {
      _lowerValue = (_upperValue - widget.maximumDistance > _fakeMin) ? _upperValue - widget.maximumDistance : _fakeMin;
      validMove = false;
      _updateLowerValue(_lowerValue);
    }
    ...
    if (widget.ignoreSteps.length > 0) {
      for (FlutterSliderIgnoreSteps steps in widget.ignoreSteps) {
        if (!((widget.rtl == false && (__rAxis >= steps.from && __rAxis <= steps.to) == false) || (widget.rtl == true && ((_fakeMax - __rAxis) >= steps.from && (_fakeMax - __rAxis) <= steps.to) == false))) {
          validMove = false;
        }
      }
    }

    if (validMove && __axisPosTmp - (_touchSize) <= __rightHandlerPosition + 1 && __axisPosTmp + _handlersPadding >= _handlersPadding - 1 /* - _leftPadding*/
        ) {
      ...
      if (widget.jump == true) {
        double des = ((__containerSizeWithoutHandlerSize / _fakeMax) * tmpLowerValue) - (_touchSize);

        if (__middle <= des + ((__handlerSize > _touchSize) ? __handlerSize : _touchSize) && (__middle >= des - ((__handlerSize > _touchSize) ? __handlerSize : _touchSize))) {
          _lowerValue = tmpLowerValue;
...
//        }
  ...
  void _rightHandlerMove(PointerEvent pointer, [double tappedPositionWithPadding = 0]) {
    ...
    if (widget.rangeSlider && widget.minimumDistance > 0 && (__rAxis - widget.minimumDistance) <= _lowerValue) {
      _upperValue = (_lowerValue + widget.minimumDistance < _fakeMax) ? _lowerValue + widget.minimumDistance : _fakeMax;
      validMove = false;
      _updateUpperValue(_upperValue);
    }
    if (widget.rangeSlider && widget.maximumDistance > 0 && __rAxis >= (_lowerValue + widget.maximumDistance)) {
      _upperValue = (_lowerValue + widget.maximumDistance < _fakeMax) ? _lowerValue + widget.maximumDistance : _fakeMax;
      validMove = false;
      _updateUpperValue(_upperValue);
    }
    ...
    if (validMove && __axisPosTmp >= __leftHandlerPosition - 1 + (_touchSize) && __axisPosTmp + _handlersPadding <= __containerSizeWithoutHalfPadding) {
      ...
      if (widget.jump == true) {
        double des = ((__containerSizeWithoutHandlerSize / _fakeMax) * tmpUpperValue) - (_touchSize);
...
//                            color: Colors.blue.withOpacity(0.7),
            ...
            child: Listener(
              onPointerDown: (_) {
                if (widget.selectByTap) {
                  double distanceFromLeftHandler, distanceFromRightHandler, tappedPositionWithPadding;

                  if (widget.axis == Axis.horizontal) {
                    tappedPositionWithPadding = _handlersPadding + (_touchSize) - xDragTmp;
                    distanceFromLeftHandler = ((_leftHandlerXPosition + _handlersPadding + (_touchSize)) + _containerLeft - _.position.dx).abs();
                    distanceFromRightHandler = ((_rightHandlerXPosition + _handlersPadding + (_touchSize)) + _containerLeft - _.position.dx).abs();
                  } else {
                    tappedPositionWithPadding = _handlersPadding + (_touchSize) - yDragTmp;
                    distanceFromLeftHandler = ((_leftHandlerYPosition + _handlersPadding + (_touchSize)) + _containerTop - _.position.dy).abs();
                    distanceFromRightHandler = ((_rightHandlerYPosition + _handlersPadding + (_touchSize)) + _containerTop - _.position.dy).abs();
                  }
  ...
  Positioned _tooltip({String side, double value, double opacity, Animation animation}) {
    ...
    if (_tooltipData.numberFormat != null) numberFormat = _tooltipData.numberFormat.format(value);
  ...
  Positioned _activeTrack() {
    ...
    if (widget.axis == Axis.horizontal) {
      ...
      if (widget.rtl == true && widget.rangeSlider == false) {
        ...
        width = _constraintMaxWidth - _rightHandlerXPosition - _handlersWidth / 2 - (_touchSize);
      }
    } else {
      right = 0;
      ...
      if (widget.rtl == true && widget.rangeSlider == false) {
        ...
        height = _constraintMaxHeight - _rightHandlerYPosition - _handlersHeight / 2 - (_touchSize);
      }
    }
...
class _MakeHandler extends StatelessWidget {
  ...
  _MakeHandler({this.id, this.handlerData, this.visibleTouchArea, this.width, this.height, this.animation, this.axis});
        ...
        Center(
          child: ScaleTransition(
            ...
            child: handlerData.child ??
                Container(
                  alignment: Alignment.center,
                  decoration: BoxDecoration(boxShadow: [BoxShadow(color: Colors.black26, blurRadius: 2, spreadRadius: 0.2, offset: Offset(0, 1))], color: Colors.white, shape: BoxShape.circle),
                  width: width,
                  height: height,
...
class FlutterSliderHandler {
  ...
  FlutterSliderHandler({this.child, this.icon, this.disabled = false}) : assert(disabled != null);
...
class FlutterSliderTooltip {
  ...
  FlutterSliderTooltip({this.textStyle, this.boxStyle, this.leftPrefix, this.leftSuffix, this.rightPrefix, this.rightSuffix, this.numberFormat, this.alwaysShowTooltip, this.disabled});

  @override
  String toString() {
    return textStyle.toString() + boxStyle.toString() + leftPrefix.toString() + leftSuffix.toString() + rightPrefix.toString() + rightSuffix.toString() + numberFormat.toString() + alwaysShowTooltip.toString() + disabled.toString();
  }
}
...
class FlutterSliderTrackBar {
  ...
  })  : assert(activeTrackBarHeight != null && activeTrackBarHeight > 0 && inactiveTrackBarHeight != null && inactiveTrackBarHeight > 0 && activeTrackBarHeight >= inactiveTrackBarHeight),
...
class FlutterSliderHandlerAnimation {
  ...
  const FlutterSliderHandlerAnimation({this.curve = Curves.elasticOut, this.reverseCurve, this.duration = const Duration(milliseconds: 700), this.scale = 1.3}) : assert(curve != null && duration != null && scale != null);
}
